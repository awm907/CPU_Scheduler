...
public class CPUScheduler{
	public static void main(String[] args){
		...
		//Variables to save the total times for FCFS.
		float ttTimeFCFS=0;
		float twTimeFCFS=0;
		float trTimeFCFS=0;
		
		//Variables to save the total times for RR.
		float ttTimeRR=0;
		float twTimeRR=0;
		float trTimeRR=0;
		
		
		//Reading the file line by line and adding the text to "text" string.
		try{
			File file2=new File(file);
			inStream=new Scanner(file2);
			while(inStream.hasNextLine()){
				//In order to split the two processes, we are inserting a hyphen (-) between them.
		...
		//Splitting the text by the hyphen(-) and storing it in an array so that each index will have a process.
		...
		//Our first priority is to sort the array in processArray according to their arrival time.
		sort(processArray);
		System.out.println("\n-------------------------------------------------");
		...
		//Printing out the run times of each process and calculating their turnaround times, wait times and response times.  
		...
		//Calculating Average Turnaround, wait and response times.
		System.out.println();
		...
		//Resetting current time to zero for RR.
		currentTime=0;
		
		//This variable indicates how many processes are left to be executed.
		...
		while(totalProcessInRR>0){
			for(int i=0;i<processArray.length;i++){
				...
				//To check if the process has already been executed or not.
				...
				if(processArray[i].track>=timeSlice && processArray[i].aTime<=currentTime){
					...
					//If process has finished, calculating tTime2 & wTime2 for that process.
					...
					//Lastly, the prcoess is running, calculating its tTime2 & wTime2.
					processArray[i].tTime2=currentTime-processArray[i].aTime;
					processArray[i].wTime2=processArray[i].tTime2-processArray[i].bTime;
					ttTimeRR=ttTimeRR+processArray[i].tTime2;
					twTimeRR=twTimeRR+processArray[i].wTime2;
		...
		System.out.println("\n-------------------------------------------------");
		System.out.println("Project done by [Abdul Wahab Malik]");
		System.out.println("-------------------------------------------------\n");
		
		
	}
	
	//For sorting the processes array.
